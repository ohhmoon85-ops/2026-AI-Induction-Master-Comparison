...
export const SimulationPanel: React.FC = () => {
  ...
  const fetchReport = useCallback(async (task: string) => {
    ...
    if (!apiKey) {
      setResult("보고서 생성 실패: API 키를 찾을 수 없습니다.");
      return;
    }
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `
          ...
          ai-induction(특허 10-2708883)의 '직접 온도 센싱'이 삼성/LG의 간접 방식 대비 얼마나 정확하고 안전한지 숫자로 강조해줘.
  ...
  const VirtualBurner = ({ state, brand, colorClass, icon: Icon }: { state: CookState, brand: string, colorClass: string, icon: any }) => (
    <div className="flex-1 flex flex-col items-center bg-slate-900 p-5 rounded-[2rem] border border-slate-800 relative overflow-hidden">
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-100">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => startSimulation('stew')} disabled={isSimulating} className="flex-1 min-w-[100px] px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold hover:bg-slate-200">찌개</button>
            <button onClick={() => startSimulation('steak')} disabled={isSimulating} className="flex-1 min-w-[100px] px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold hover:bg-slate-200">스테이크</button>
            <button onClick={() => startSimulation('fish')} disabled={isSimulating} className="flex-1 min-w-[100px] px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold hover:bg-slate-200">튀김</button>
          </div>
        </div>
      ...
      {isProcessingReport && (
        <div className="py-16 bg-white rounded-3xl border border-slate-100 flex flex-col items-center">
          <Activity className="w-10 h-10 animate-pulse text-indigo-500 mb-4" />
      ...
      {result && (
        <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-slate-100 animate-in slide-in-from-bottom-4">
          ...
          <button 
            ...
            className="mt-8 w-full py-3 text-slate-400 text-[10px] font-black uppercase tracking-widest"
          >
            닫기
          </button>
        </div>
